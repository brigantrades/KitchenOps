...
/// fun salad ideas page — aligns with Discover "Salads" (`lunch-salads`).
///
/// Source: https://www.food.com/ideas/fun-salad-ideas-6301
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to salads, slaws,
/// and salad dressings from this list.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 50 — hub lists ~45)
///
/// Dry run writes tmp/foodcom_lunch_salads_fun_review.csv.
...
const _ideasPageUrl = 'https://www.food.com/ideas/fun-salad-ideas-6301';
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverFunSaladUrls(client, limit: limit);
...
Future<List<String>> _discoverFunSaladUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyFunSaladHubRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep salads, slaws & dressings.
bool _isLikelyFunSaladHubRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cat.contains('salad')) return true;
  if (haystack.contains('salad')) return true;
  if (haystack.contains('slaw') || haystack.contains('coleslaw')) return true;
  if (haystack.contains('tabbouleh')) return true;
  if (haystack.contains('vinaigrette')) return true;
  if (title.contains('dressing')) return true;
  if (title.contains('crouton')) return true;

  return false;
}

/// Tags tuned for [kBrowseCategories] `lunch-salads` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('vinaigrette') || t.contains('vinaigrette')) {
    tags.add('vinaigrette');
  }
  if (haystack.contains('greens') || haystack.contains('mixed green')) {
    tags.add('greens');
  }
  if (haystack.contains('caesar') || t.contains('caesar')) {
    tags.add('caesar');
  }
  if (haystack.contains('grain bowl') ||
      (haystack.contains('quinoa') && haystack.contains('bowl'))) {
    tags.add('grain bowl');
  }
  if (haystack.contains('dressing') || t.contains('dressing')) {
    tags.add('salad dressing');
  }
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_lunch_salads_fun_review.csv';
