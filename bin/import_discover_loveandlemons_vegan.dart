...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverVeganUrls(client, limit: limit);
...
Future<List<String>> _discoverVeganUrls(
  http.Client client, {
  required int limit,
}) async {
  const hub = 'https://www.loveandlemons.com/vegan-recipes/';
  final response = await client.get(Uri.parse(hub));
  if (response.statusCode < 200 || response.statusCode >= 300) {
    throw Exception('Could not fetch vegan hub page: ${response.statusCode}');
  }
  final urls = _extractLoveAndLemonsRecipeUrls(response.body);
  ...
  if (recipeUrls.isEmpty) {
    throw Exception('No vegan recipe URLs discovered from source page.');
  }
...
List<String> _extractLoveAndLemonsRecipeUrls(String html) {
  ...
  final focusBlockMatch = RegExp(
    r'85 Best Vegan Recipes([\s\S]*?)Looking for more vegan recipes\?',
    caseSensitive: false,
  ).firstMatch(html);
  ...
  for (final m in abs.allMatches(source)) {
    ...
    if (!_isLikelyRecipePath(url)) continue;
    out.add(url.endsWith('/') ? url : '$url/');
  }
  final rel = RegExp(r'href="(/[^"#? ]+)"', caseSensitive: false);
  for (final m in rel.allMatches(source)) {
...
bool _isLikelyRecipePath(String url) {
...
Map<String, dynamic> _toPayload({
  ...
  return <String, dynamic>{
    ...
    'meal_type': 'sauce',
    'cuisine_tags': const <String>['Plant-Based Power', 'Vegan Delights'],
    ...
    'visibility': 'public',
    'is_public': true,
    'source': 'love_and_lemons',
    'source_url': sourceUrl,
    'api_id': 'love_and_lemons:$slug',
  };
}
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  final path = '${dir.path}/loveandlemons_vegan_review.csv';
