...
/// Latin American recipe ideas page — aligns with Discover "Latin American"
/// (`dinner-latin-american`).
///
/// Source: https://www.food.com/ideas/best-latin-american-recipes-7133
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Latin American picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 42 — hub lists ~39)
///
/// Dry run writes tmp/foodcom_dinner_latin_american_review.csv.
...
const _latinAmericanSignals = <String>[
  'latin american',
  'latino',
  'latam',
  'south american',
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 42;
  ...
  try {
    ...
    final urls = await _discoverRegionalLatinAmericanUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalLatinAmericanUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalLatinAmericanRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Latin American picks.
bool _isLikelyRegionalLatinAmericanRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('latin american') ||
      cuisine.contains('south american') ||
      cuisine.contains('central american') ||
      cuisine.contains('caribbean') ||
      cuisine.contains('mexican') ||
      cuisine.contains('brazilian') ||
      cuisine.contains('peruvian') ||
      cuisine.contains('colombian') ||
      cuisine.contains('cuban') ||
      cuisine.contains('argentinian') ||
      cuisine.contains('argentine') ||
      cuisine.contains('chilean') ||
      cuisine.contains('ecuadorian') ||
      cuisine.contains('guatemalan') ||
      cuisine.contains('honduran') ||
      cuisine.contains('salvadoran') ||
      cuisine.contains('nicaraguan') ||
      cuisine.contains('costa rican') ||
      cuisine.contains('panamanian') ||
      cuisine.contains('bolivian') ||
      cuisine.contains('paraguayan') ||
      cuisine.contains('uruguayan') ||
      cuisine.contains('venezuelan') ||
      cuisine.contains('puerto rican') ||
      cuisine.contains('dominican')) {
    return true;
  }

  for (final s in _latinAmericanSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-latin-american` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('ceviche')) tags.add('ceviche');
  if (haystack.contains('empanada')) tags.add('empanada');
  if (haystack.contains('pernil')) tags.add('pernil');
  if (haystack.contains('sofrito')) tags.add('sofrito');
  if (haystack.contains('gallo pinto')) tags.add('gallo pinto');
  if (haystack.contains('moqueca')) tags.add('moqueca');
  if (haystack.contains('pupusa')) tags.add('pupusa');
...
Map<String, dynamic> _toPayload({
  ...
  if (imageUrl.isEmpty) {
    imageUrl =
        'https://images.unsplash.com/photo-1544025162-76694262813d?auto=format&fit=crop&w=1200&q=80';
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_latin_american_review.csv';
