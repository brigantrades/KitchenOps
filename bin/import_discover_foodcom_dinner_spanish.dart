...
/// Spanish food at-home ideas page — aligns with Discover "Spanish"
/// (`dinner-spanish`).
///
/// Source: https://www.food.com/ideas/spanish-food-recipes-at-home-7122
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Spanish picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 52 — hub lists ~50)
///
/// Dry run writes tmp/foodcom_dinner_spanish_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 52;
  ...
  try {
    ...
    final urls = await _discoverRegionalSpanishUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalSpanishUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalSpanishRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Spanish picks.
bool _isLikelyRegionalSpanishRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('spanish')) return true;

  for (final s in _spanishSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-spanish` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('paella')) tags.add('paella');
  if (haystack.contains('tapas') || haystack.contains('tapa')) tags.add('tapas');
  if (haystack.contains('gazpacho')) tags.add('gazpacho');
  if (haystack.contains('chorizo')) tags.add('chorizo');
  if (haystack.contains('manchego')) tags.add('manchego');
  if (haystack.contains('patatas')) tags.add('patatas');
  if (haystack.contains('sangria')) tags.add('sangria');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_spanish_review.csv';
