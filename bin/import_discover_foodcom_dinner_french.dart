...
/// French food at-home ideas page — aligns with Discover "French"
/// (`dinner-french`).
///
/// Source: https://www.food.com/ideas/french-food-at-home-7129
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to French picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 64 — hub lists ~62)
///
/// Dry run writes tmp/foodcom_dinner_french_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 64;
  ...
  try {
    ...
    final urls = await _discoverRegionalFrenchUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalFrenchUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalFrenchRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep French picks.
bool _isLikelyRegionalFrenchRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('french')) return true;

  for (final s in _frenchSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-french` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('ratatouille')) tags.add('ratatouille');
  if (haystack.contains('bourguignon')) tags.add('bourguignon');
  if (haystack.contains('quiche')) tags.add('quiche');
  if (haystack.contains('gratin')) tags.add('gratin');
  if (haystack.contains('coq au vin')) tags.add('coq au vin');
  if (haystack.contains('bistro')) tags.add('bistro');
  if (haystack.contains('cassoulet')) tags.add('cassoulet');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_french_review.csv';
