...
/// Japanese food at-home ideas page — aligns with Discover "Japanese"
/// (`dinner-japanese`).
///
/// Source: https://www.food.com/ideas/japanese-food-recipes-at-home-7140
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Japanese picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 55 — hub lists ~50+)
///
/// Dry run writes tmp/foodcom_dinner_japanese_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalJapaneseUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalJapaneseUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalJapaneseRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Japanese picks.
bool _isLikelyRegionalJapaneseRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('japanese')) return true;

  for (final s in _japaneseSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-japanese` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('ramen')) tags.add('ramen');
  if (haystack.contains('teriyaki')) tags.add('teriyaki');
  if (haystack.contains('sushi')) tags.add('sushi');
  if (haystack.contains('miso')) tags.add('miso');
  if (haystack.contains('udon')) tags.add('udon');
  if (haystack.contains('soba')) tags.add('soba');
  if (haystack.contains('tempura')) tags.add('tempura');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_japanese_review.csv';
