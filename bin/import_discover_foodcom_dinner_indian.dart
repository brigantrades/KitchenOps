...
/// Indian food at-home ideas page — aligns with Discover "Indian"
/// (`dinner-indian`).
///
/// Source: https://www.food.com/ideas/indian-food-recipes-at-home-6821
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Indian picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 85 — hub lists ~82)
///
/// Dry run writes tmp/foodcom_dinner_indian_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 85;
  ...
  try {
    ...
    final urls = await _discoverRegionalIndianUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalIndianUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalIndianRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Indian picks.
bool _isLikelyRegionalIndianRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('indian')) return true;

  for (final s in _indianSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-indian` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('tikka')) tags.add('tikka');
  if (haystack.contains('masala')) tags.add('masala');
  if (haystack.contains('korma')) tags.add('korma');
  if (haystack.contains('curry')) tags.add('curry');
  if (RegExp(r'\bdal\b').hasMatch(haystack) || haystack.contains('daal')) {
    tags.add('dal');
  }
  if (haystack.contains('biryani')) tags.add('biryani');
  if (haystack.contains('naan')) tags.add('naan');
  if (haystack.contains('tandoori')) tags.add('tandoori');
...
Map<String, dynamic> _toPayload({
  ...
  if (imageUrl.isEmpty) {
    imageUrl =
        'https://images.unsplash.com/photo-1585937421612-70a008216fbe?auto=format&fit=crop&w=1200&q=80';
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_indian_review.csv';
