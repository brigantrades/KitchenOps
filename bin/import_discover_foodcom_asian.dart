...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverAsianRecipeUrls(client, limit: limit);
...
Future<List<String>> _discoverAsianRecipeUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  final pages = <String>[
    'https://www.food.com/search/Asian+Entree?pn=3',
    'https://www.food.com/search/Asian+Entree?pn=2',
    'https://www.food.com/search/Asian+Entree?pn=1',
    'https://www.food.com/ideas/asian-food-recipes-at-home-7133',
    'https://www.food.com/ideas/chinese-food-at-home-6807',
    'https://www.food.com/ideas/japanese-food-recipes-at-home-7140',
    'https://www.food.com/ideas/korean-recipes-6990',
    'https://www.food.com/ideas/thai-recipes-6843',
  ];
  ...
  for (final pageUrl in pages) {
    try {
      ...
      final urls = _extractFoodComRecipeUrls(response.body).toList()..sort();
      for (final candidate in urls) {
        if (discovered.length >= limit) break;
        if (reviewed.contains(candidate)) continue;
        reviewed.add(candidate);
        final recipeId = _recipeIdFromUrl(candidate);
        if (recipeId != null && seenRecipeIds.contains(recipeId)) {
          continue;
        }
        try {
          final recipeJsonLd = await _fetchRecipeJsonLd(client, candidate);
          if (_isLikelyAsianRecipe(recipeJsonLd)) {
...
bool _isLikelyAsianRecipe(Map<String, dynamic> jsonLd) {
  final title = (jsonLd['name']?.toString() ?? '').toLowerCase();
  final description = (jsonLd['description']?.toString() ?? '').toLowerCase();
  final cuisine = (jsonLd['recipeCuisine']?.toString() ?? '').toLowerCase();
  final keywords = (jsonLd['keywords']?.toString() ?? '').toLowerCase();
  final haystack = '$title $description $cuisine $keywords';
  ...
  final hasPositive = positive.any(haystack.contains);
  final hasNegative = negative.any(haystack.contains);
...
Map<String, dynamic> _toPayload({
  ...
  return <String, dynamic>{
    ...
    'meal_type': 'sauce',
    'cuisine_tags': const <String>['Asian'],
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  final path = '${dir.path}/foodcom_asian_review.csv';
