...
/// Imports public recipes from Food.com's "Top 100 Recipes For a Crowd" hub —
/// aligns with Discover "Crowd Favorites" (`snack-crowd-favorites`).
///
/// Source: https://www.food.com/ideas/top-recipes-for-a-crowd-7005
///
/// Hub mixes mains, sides, apps, and sweets; all rows use [meal_type] `snack` so
/// they surface under Discover Appetizers (same as other snack browse imports).
/// Every row is tagged `crowd favorites` for browse matching.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 100)
///
/// Dry run writes tmp/foodcom_snack_crowd_favorites_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/top-recipes-for-a-crowd-7005';
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 100;
  ...
  try {
    ...
    final urls = await _discoverRegionalCrowdFavoriteUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalCrowdFavoriteUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalCrowdFavoriteRecipe(jsonLd)) continue;
...
/// Hub page is a curated "for a crowd" list; accept any valid Recipe JSON-LD.
bool _isLikelyRegionalCrowdFavoriteRecipe(Map<String, dynamic> _) {
  return true;
}

/// Tags tuned for [kBrowseCategories] `snack-crowd-favorites` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('potluck')) tags.add('potluck');
  if (haystack.contains('party')) tags.add('party');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_snack_crowd_favorites_review.csv';
