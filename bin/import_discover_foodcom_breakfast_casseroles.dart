...
/// Imports public Breakfast (meal_type entree) recipes from Food.com's
/// breakfast casserole ideas page — aligns with Discover "Casseroles"
/// (`breakfast-casseroles`) browse keywords.
///
/// Source: https://www.food.com/ideas/breakfast-casserole-recipes-6531
///
/// Discovery filters JSON-LD: the hub page lists many unrelated `/recipe/` links;
/// we only keep recipes whose title/keywords/category look like a casserole or
/// related baked breakfast (see [_isLikelyBreakfastCasseroleRecipe]).
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 45)
///
/// Dry run writes tmp/foodcom_breakfast_casseroles_review.csv and does not write DB.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/breakfast-casserole-recipes-6531';
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 45;
  ...
  try {
    ...
    final urls = await _discoverBreakfastCasseroleUrls(client, limit: limit);
...
Future<List<String>> _discoverBreakfastCasseroleUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  final rawUrls = _extractFoodComRecipeUrls(response.body);
  final reviewed = <String>{};
  final seenRecipeKeys = <String>{};
  final discovered = <String>[];

  final sorted = rawUrls.toList()..sort();
  for (final candidate in sorted) {
    ...
    try {
      ...
      if (!_isLikelyBreakfastCasseroleRecipe(jsonLd)) continue;
...
/// The ideas page HTML includes many extra `/recipe/` links (related stories, hubs).
/// We gate on JSON-LD so unrelated egg tutorials (e.g. Turkish eggs, scrambled eggs)
/// are not imported as casseroles.
bool _isLikelyBreakfastCasseroleRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (title.contains('turkish') && title.contains('egg')) return false;
  if (title.contains('shakshuka')) return false;
  if (RegExp(r'how to (make|cook)\s+scrambled').hasMatch(title)) return false;
  if ((title.contains('scrambled egg') || title.contains('scrambled eggs')) &&
      !haystack.contains('casserole')) {
    return false;
  }

  if (haystack.contains('casserole')) return true;
  if (haystack.contains('strata')) return true;
  if (haystack.contains('egg bake')) return true;
  if (haystack.contains('breakfast bake')) return true;
  if (haystack.contains('dutch baby')) return true;
  if (title.contains('german') &&
      (title.contains('pancake') ||
          title.contains('puff') ||
          title.contains('baked'))) {
    return true;
  }
  if (haystack.contains('french toast') &&
      (haystack.contains('bake') || haystack.contains('casserole'))) {
    return true;
  }
  if ((haystack.contains('hash brown') || haystack.contains('hashbrown')) &&
      (haystack.contains('casserole') || haystack.contains('bake'))) {
    return true;
  }
  if (haystack.contains('tater tot') || haystack.contains('tater-tot')) {
    return true;
  }
  if (haystack.contains('cinnamon roll') && haystack.contains('casserole')) {
    return true;
  }
  if (haystack.contains('biscuit') &&
      haystack.contains('gravy') &&
      haystack.contains('casserole')) {
    return true;
  }
  if (haystack.contains('slow-cooker') && haystack.contains('casserole')) {
    return true;
  }
  if (haystack.contains('crock') && haystack.contains('casserole')) return true;

  return false;
}

/// Tags tuned for [kBrowseCategories] `breakfast-casseroles` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('strata')) tags.add('strata');
  if (haystack.contains('egg bake')) tags.add('egg bake');
  if (haystack.contains('tater tot') || haystack.contains('tater-tot')) {
    tags.add('tater tot');
  }
  if (haystack.contains('hash brown')) tags.add('hash brown');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_breakfast_casseroles_review.csv';
