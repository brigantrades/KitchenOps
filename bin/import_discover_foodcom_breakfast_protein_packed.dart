...
/// protein-packed breakfasts ideas page — aligns with Discover "Protein Packed"
/// (`breakfast-protein-packed`).
///
/// Source: https://www.food.com/ideas/protein-packed-breakfasts-6450
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to breakfast-style
/// picks (eggs, meat, protein bars, shakes, etc.).
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 20 — hub lists ~18)
///
/// Dry run writes tmp/foodcom_breakfast_protein_packed_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/protein-packed-breakfasts-6450';
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 20;
  ...
  try {
    ...
    final urls = await _discoverProteinPackedBreakfastUrls(client, limit: limit);
...
Future<List<String>> _discoverProteinPackedBreakfastUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyProteinPackedBreakfastRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep this hub’s breakfast picks.
bool _isLikelyProteinPackedBreakfastRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cat.contains('dessert') && !title.contains('breakfast')) return false;

  final looksBreakfast = cat.contains('breakfast') ||
      cat.contains('brunch') ||
      title.contains('breakfast') ||
      title.contains('brunch') ||
      haystack.contains('morning') ||
      haystack.contains('protein') ||
      RegExp(r'\beggs?\b').hasMatch(haystack) ||
      ...
      haystack.contains('granola') ||
      haystack.contains('sausage') ||
      haystack.contains('bacon') ||
      haystack.contains('stromboli') ||
      haystack.contains('egg roll') ||
      haystack.contains('sandwich') ||
      haystack.contains('biscuit') ||
      haystack.contains('bars') && haystack.contains('peanut');

  return looksBreakfast;
}

/// Tags tuned for [kBrowseCategories] `breakfast-protein-packed` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('protein bar') || title.toLowerCase().contains('protein bar')) {
    tags.add('protein bar');
  }
  if (haystack.contains('smoothie') || haystack.contains('shake')) {
    tags.add('protein shake');
  }
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_breakfast_protein_packed_review.csv';
