...
/// breakfast smoothies ideas page — aligns with Discover "Smoothies & Bowls"
/// (`breakfast-smoothies`): smoothie, acai, bowl, shake, blend.
///
/// Source: https://www.food.com/ideas/11-breakfast-smoothies-6100
///
/// Hub pages embed extra recipe links; JSON-LD is filtered to real smoothies.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 12 — matches the hub list size)
///
/// Dry run writes tmp/foodcom_breakfast_smoothies_review.csv and does not write DB.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/11-breakfast-smoothies-6100';
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 12;
  ...
  try {
    ...
    final urls = await _discoverBreakfastSmoothieUrls(client, limit: limit);
...
Future<List<String>> _discoverBreakfastSmoothieUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyBreakfastSmoothieRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep blended breakfast drinks.
bool _isLikelyBreakfastSmoothieRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (title.contains('dr. oz')) return false;
  if (title.contains('prenatal')) return false;

  if (haystack.contains('smoothie')) return true;
  if (cat.contains('smoothie')) return true;
  if (title.contains('julius') && title.contains('orange')) return true;
  if (haystack.contains('acai') && haystack.contains('bowl')) return true;
  return false;
}

/// Tags tuned for [kBrowseCategories] `breakfast-smoothies` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('acai')) tags.add('acai');
  if (haystack.contains('bowl')) tags.add('bowl');
  if (haystack.contains('shake')) tags.add('shake');
  if (haystack.contains('blend')) tags.add('blend');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_breakfast_smoothies_review.csv';
