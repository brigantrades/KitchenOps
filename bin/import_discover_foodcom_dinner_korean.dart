...
/// Korean food at-home ideas page — aligns with Discover "Korean"
/// (`dinner-korean`).
///
/// Source: https://www.food.com/ideas/korean-food-recipes-at-home-7143
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Korean picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 32 — hub lists ~30)
///
/// Dry run writes tmp/foodcom_dinner_korean_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalKoreanUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalKoreanUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalKoreanRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Korean picks.
bool _isLikelyRegionalKoreanRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('korean')) return true;

  for (final s in _koreanSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-korean` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('bibimbap')) tags.add('bibimbap');
  if (haystack.contains('kimchi')) tags.add('kimchi');
  if (haystack.contains('bulgogi')) tags.add('bulgogi');
  if (haystack.contains('gochujang')) tags.add('gochujang');
  if (haystack.contains('japchae')) tags.add('japchae');
...
Map<String, dynamic> _toPayload({
  ...
  if (imageUrl.isEmpty) {
    imageUrl =
        'https://images.unsplash.com/photo-1590301157890-4806928fcc2f?auto=format&fit=crop&w=1200&q=80';
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_korean_review.csv';
