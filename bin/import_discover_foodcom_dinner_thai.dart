...
/// Thai food at-home ideas page — aligns with Discover "Thai"
/// (`dinner-thai`).
///
/// Source: https://www.food.com/ideas/thai-food-recipes-at-home-6820
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Thai picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 55 — hub lists ~53)
///
/// Dry run writes tmp/foodcom_dinner_thai_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalThaiUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalThaiUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalThaiRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Thai picks.
bool _isLikelyRegionalThaiRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('thai')) return true;

  for (final s in _thaiSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-thai` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('pad thai')) tags.add('pad thai');
  if (haystack.contains('coconut') && haystack.contains('curry')) {
    tags.add('coconut curry');
  }
  if (haystack.contains('lemongrass')) tags.add('lemongrass');
  if (haystack.contains('thai curry') ||
      (haystack.contains('curry') &&
          (cuisine.contains('thai') || t.contains('thai')))) {
    tags.add('thai curry');
  }
  if (haystack.contains('thai basil')) tags.add('thai basil');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_thai_review.csv';
