...
/// Chinese food at-home ideas page — aligns with Discover "Chinese"
/// (`dinner-chinese`).
///
/// Source: https://www.food.com/ideas/chinese-food-at-home-6807
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Chinese picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 55 — hub lists ~52)
///
/// Dry run writes tmp/foodcom_dinner_chinese_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalChineseUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalChineseUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalChineseRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Chinese picks.
bool _isLikelyRegionalChineseRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('chinese')) return true;

  for (final s in _chineseSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-chinese` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('stir-fry')) tags.add('stir-fry');
  if (haystack.contains('stir fry')) tags.add('stir fry');
  if (haystack.contains('lo mein')) tags.add('lo mein');
  if (haystack.contains('fried rice')) tags.add('fried rice');
  if (haystack.contains('kung pao') || haystack.contains('gong bao')) {
    tags.add('kung pao');
  }
  if (haystack.contains('szechuan') || haystack.contains('sichuan')) {
    tags.add('szechuan');
  }
  if (haystack.contains('wok')) tags.add('wok');
...
Map<String, dynamic> _toPayload({
  ...
  if (imageUrl.isEmpty) {
    imageUrl =
        'https://images.unsplash.com/photo-1563245379-fdf02464b1f2?auto=format&fit=crop&w=1200&q=80';
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_chinese_review.csv';
