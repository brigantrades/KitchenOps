...
/// five-ingredient lunch ideas page — aligns with Discover "5 Ingredients or Less"
/// (`lunch-five-ingredients`).
///
/// Source: https://www.food.com/ideas/five-ingredient-lunches-6084
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to lunch-style picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 45 — hub lists ~40)
///
/// Dry run writes tmp/foodcom_lunch_five_ingredients_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverFiveIngredientLunchUrls(client, limit: limit);
...
Future<List<String>> _discoverFiveIngredientLunchUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyFiveIngredientLunchRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep this hub’s lunch picks.
bool _isLikelyFiveIngredientLunchRecipe(Map<String, dynamic> jsonLd) {
  ...
  final lunchish = cat.contains('lunch') ||
      ...
      title.contains('slow-cooker') ||
      title.contains('boston') ||
      title.contains('noodle') ||
      haystack.contains('five-ingredient') ||
...
/// Tags tuned for [kBrowseCategories] `lunch-five-ingredients` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('5-ingredient') || haystack.contains('five-ingredient')) {
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_lunch_five_ingredients_review.csv';
