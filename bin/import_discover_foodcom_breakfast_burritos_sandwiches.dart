...
/// Imports public Breakfast (meal_type entree) recipes from Food.com ideas
/// pages — aligns with Discover "Burritos & Sandwiches" (`breakfast-savory`).
///
/// Sources:
/// - https://www.food.com/ideas/breakfast-sandwiches-6529
/// - https://www.food.com/ideas/breakfast-burrito-recipes-7094
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to handheld
/// breakfast sandwiches, burritos, wraps, etc.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 72 — ~28 + ~44 hub items)
///
/// Dry run writes tmp/foodcom_breakfast_burritos_sandwiches_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 72;
  ...
  try {
    ...
    final urls = await _discoverBurritoSandwichUrls(client, limit: limit);
...
Future<List<String>> _discoverBurritoSandwichUrls(
  http.Client client, {
  required int limit,
}) async {
  final mergedOrder = <String>[];
  final seenUrls = <String>{};
  for (final hub in _ideasPageUrls) {
    final response = await client.get(Uri.parse(hub));
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'Ideas page fetch failed (${response.statusCode}) for $hub',
      );
    }
    for (final u in _extractFoodComRecipeUrlsInDocumentOrder(response.body)) {
      if (seenUrls.add(u)) mergedOrder.add(u);
    }
  }
  ...
  for (final candidate in mergedOrder) {
    ...
    try {
      ...
      if (!_isLikelyBurritoSandwichRecipe(jsonLd)) continue;
  ...
  if (discovered.isEmpty) {
    throw Exception(
      'No Food.com recipe URLs discovered. The ideas pages may require '
...
/// Ideas pages include unrelated `/recipe/` links; keep handheld breakfast items.
bool _isLikelyBurritoSandwichRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (haystack.contains('smoothie')) return false;

  if (title.contains('monte cristo')) return true;
  if (haystack.contains('croque')) return true;
  if (RegExp(r'breakfast\s+basket').hasMatch(haystack)) return true;

  final hasHandheld = haystack.contains('burrito') ||
      haystack.contains('sandwich') ||
      haystack.contains('panini') ||
      haystack.contains('stromboli') ||
      RegExp(r'\bwrap\b').hasMatch(haystack) ||
      haystack.contains('melt') ||
      haystack.contains('melts') ||
      haystack.contains('chimichanga') ||
      haystack.contains('mcmuffin') ||
      haystack.contains('mc muffin') ||
      haystack.contains('applewich') ||
      (haystack.contains('grilled cheese') &&
          (haystack.contains('breakfast') ||
              haystack.contains('egg') ||
              title.contains('breakfast')));

  if (!hasHandheld) return false;

  if (haystack.contains('casserole') &&
      !haystack.contains('sandwich') &&
      !haystack.contains('burrito') &&
      !RegExp(r'\bwrap\b').hasMatch(haystack) &&
      !haystack.contains('panini')) {
    return false;
  }
...
/// Tags tuned for [kBrowseCategories] `breakfast-savory` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (RegExp(r'\bburrito\b').hasMatch(haystack)) {
    tags.add('burrito');
    tags.add('breakfast burrito');
  }
  if (RegExp(r'\bsandwich\b').hasMatch(haystack) || t.contains('sandwich')) {
    tags.add('sandwich');
    tags.add('breakfast sandwich');
  }
  if (t.contains('monte cristo')) {
    tags.add('sandwich');
    tags.add('breakfast sandwich');
  }
  if (RegExp(r'\bwrap\b').hasMatch(haystack)) tags.add('wrap');
  if (haystack.contains('panini')) tags.add('panini');
  if (haystack.contains('melt') || haystack.contains('melts')) tags.add('melt');
  if (haystack.contains('stromboli')) tags.add('stromboli');
  if (haystack.contains('chimichanga')) tags.add('chimichanga');
  if (haystack.contains('croque')) tags.add('croque');
  if (RegExp(r'breakfast\s+basket').hasMatch(haystack)) {
    tags.add('sandwich');
    tags.add('breakfast sandwich');
  }
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_breakfast_burritos_sandwiches_review.csv';
