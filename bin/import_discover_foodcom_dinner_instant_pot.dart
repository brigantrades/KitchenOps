...
/// Instant Pot ideas page — aligns with Discover "Instant Pot"
/// (`dinner-instant-pot`).
///
/// Source: https://www.food.com/ideas/best-instant-pot-recipes-6928
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to pressure-cooker picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 62 — hub lists ~59)
///
/// Dry run writes tmp/foodcom_dinner_instant_pot_review.csv.
...
const _instantPotSignals = <String>[
  'instant pot',
  'instant-pot',
  'pressure cooker',
  'electric pressure',
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 62;
  ...
  try {
    ...
    final urls = await _discoverRegionalInstantPotUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalInstantPotUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalInstantPotRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Instant Pot / pressure picks.
bool _isLikelyRegionalInstantPotRecipe(Map<String, dynamic> jsonLd) {
  ...
  for (final s in _instantPotSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-instant-pot` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('instant-pot')) tags.add('instant-pot');
  if (haystack.contains('pressure cooker')) tags.add('pressure cooker');
  if (haystack.contains('electric pressure')) tags.add('electric pressure');
  if (haystack.contains('multicooker')) tags.add('multicooker');
  if (haystack.contains('instapot')) tags.add('instapot');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_instant_pot_review.csv';
