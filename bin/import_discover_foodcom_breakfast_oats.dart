...
/// overnight oats ideas page — aligns with Discover "Oats & Granola" browse
...
///
/// Dry run writes tmp/foodcom_breakfast_oats_review.csv and does not write DB.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/overnight-oats-recipes-6764';
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverBreakfastOatsUrls(client, limit: limit);
...
Future<List<String>> _discoverBreakfastOatsUrls(
  http.Client client, {
  required int limit,
}) async {
...
/// Tags tuned for [kBrowseCategories] `breakfast-oats` (oat, overnight oats,
/// granola, porridge, muesli) plus Discover dietary substring filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('granola')) tags.add('granola');
  if (haystack.contains('porridge')) tags.add('porridge');
  if (haystack.contains('muesli')) tags.add('muesli');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_breakfast_oats_review.csv';
