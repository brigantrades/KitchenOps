...
/// Imports public dessert recipes from Food.com's easy summer pies & cobblers hub —
/// aligns with Discover "Summer Delights" (`dessert-summer-delights`).
///
/// Source: https://www.food.com/ideas/easy-summer-pies-6272
///
/// [meal_type] is `dessert`. Rows are tagged `summer delights` (and related terms)
/// for browse matching.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 40 — hub lists ~39)
///
/// Dry run writes tmp/foodcom_dessert_summer_delights_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/easy-summer-pies-6272';
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 40;
  ...
  try {
    ...
    final urls = await _discoverRegionalSummerDelightUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalSummerDelightUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalSummerDelightRecipe(jsonLd)) continue;
...
/// Curated summer pies / cobblers / crisps hub; accept any valid Recipe JSON-LD.
bool _isLikelyRegionalSummerDelightRecipe(Map<String, dynamic> _) {
  return true;
}

/// Tags tuned for [kBrowseCategories] `dessert-summer-delights` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('cobbler')) tags.add('cobbler');
  if (haystack.contains('crisp')) tags.add('crisp');
  if (haystack.contains('pie')) tags.add('pie');
  if (haystack.contains('peach')) tags.add('peach');
  if (haystack.contains('strawberry')) tags.add('strawberry');
  if (haystack.contains('blueberry')) tags.add('berry');
  if (haystack.contains('blackberry')) tags.add('berry');
  if (haystack.contains('raspberry')) tags.add('berry');
  if (haystack.contains('cherry')) tags.add('cherry');
...
Map<String, dynamic> _toPayload({
  ...
  final resolvedTitle =
      title.isNotEmpty ? title : _titleFromSlug(_slugFromUrl(sourceUrl));
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dessert_summer_delights_review.csv';
