...
/// kids school lunch ideas page — aligns with Discover "School Lunch"
/// (`lunch-school`).
///
/// Source: https://www.food.com/ideas/kids-school-lunch-ideas-6050
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to packable/kid
/// lunch picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 55 — hub lists ~53)
///
/// Dry run writes tmp/foodcom_lunch_school_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/kids-school-lunch-ideas-6050';
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverSchoolLunchUrls(client, limit: limit);
...
Future<List<String>> _discoverSchoolLunchUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelySchoolLunchRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep this hub’s school-lunch picks.
bool _isLikelySchoolLunchRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (RegExp(r'\b(wedding cake|birthday cake)\b').hasMatch(haystack)) {
    return false;
  }

  if (haystack.contains('lunchbox') ||
      haystack.contains('lunch box') ||
      RegExp(r'\bschool lunch\b').hasMatch(haystack) ||
      title.contains('school') ||
      title.contains('bento') ||
      RegExp(r'\bkid').hasMatch(title) ||
      haystack.contains('for kids')) {
    return true;
  }

  final portable = cat.contains('lunch') ||
      cat.contains('snack') ||
      title.contains('muffin') ||
      title.contains('wrap') ||
      title.contains('roll-up') ||
      title.contains('rollup') ||
      title.contains('roll up') ||
      title.contains('rollwich') ||
      title.contains('quesadilla') ||
      title.contains('pinwheel') ||
      title.contains('granola') ||
      title.contains('trail mix') ||
      title.contains('thermos') ||
      ...
      title.contains('chip') ||
      title.contains('tot') ||
      title.contains('sundae') ||
      title.contains('bowl') ||
      haystack.contains('ants on') ||
      title.contains('stacker') ||
      title.contains('fun-due') ||
      title.contains('pocket') ||
      title.contains('scroll') ||
      title.contains('tortilla') ||
      title.contains('sailboat') ||
      title.contains('grapefruit') ||
      title.contains('nemo') ||
      title.contains('clock') ||
      title.contains('split pea') ||
      title.contains('sweet potato') ||
      title.contains('cauliflower') ||
      title.contains('kale') ||
      title.contains('beet') ||
      title.contains('pasta salad') ||
      title.contains('egg salad') ||
      title.contains('orange julius') ||
      title.contains('copycat') ||
      title.contains('lunchable') ||
      (title.contains('apple') && haystack.contains('peanut')) ||
      (title.contains('sushi') && haystack.contains('peanut')) ||
      (title.contains('fruit') && title.contains('cup')) ||
      title.contains('frozen fruit') ||
      title.contains('yogurt fruit') ||
      (title.contains('zucchini') && title.contains('corn'));

  return portable;
}

/// Tags tuned for [kBrowseCategories] `lunch-school` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('bento')) tags.add('bento');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_lunch_school_review.csv';
