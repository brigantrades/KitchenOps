...
/// slow-cooker / Crock Pot ideas page — aligns with Discover "Crock Pot"
/// (`dinner-crock-pot`).
///
/// Source: https://www.food.com/ideas/slow-cooker-recipes-and-crock-pot-recipes-6017
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to slow-cooker picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 80 — hub lists ~77)
///
/// Dry run writes tmp/foodcom_dinner_crock_pot_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalCrockPotUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalCrockPotUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalCrockPotRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep slow-cooker / Crock Pot picks.
bool _isLikelyRegionalCrockPotRecipe(Map<String, dynamic> jsonLd) {
  ...
  for (final s in _crockPotSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-crock-pot` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('crockpot')) tags.add('crockpot');
  if (haystack.contains('crock-pot')) tags.add('crock-pot');
  if (haystack.contains('slow cooker')) tags.add('slow cooker');
  if (haystack.contains('slow-cooker')) tags.add('slow-cooker');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_crock_pot_review.csv';
