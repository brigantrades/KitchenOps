...
/// best dips & spreads ideas page — aligns with Discover "Dips & Spreads"
/// (`snack-dips`).
///
/// Source: https://www.food.com/ideas/best-dips-spreads-6065
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to dip / spread picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 60 — adjust to hub size)
///
/// Dry run writes tmp/foodcom_snack_dips_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/best-dips-spreads-6065';

/// Substrings for JSON-LD gating (Food.com hub is curated; list errs on inclusive).
const _dipSpreadSignals = <String>[
  'dip',
  'spread',
  'hummus',
  'guacamole',
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 60;
  ...
  try {
    ...
    final urls = await _discoverRegionalSnackDipUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalSnackDipUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalSnackDipRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep dip / spread picks.
bool _isLikelyRegionalSnackDipRecipe(Map<String, dynamic> jsonLd) {
  ...
  for (final s in _dipSpreadSignals) {
...
/// Tags tuned for [kBrowseCategories] `snack-dips` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (RegExp(r'\bdip\b').hasMatch(haystack) || haystack.contains('dip,')) {
    tags.add('dip');
  }
  if (haystack.contains('spread')) tags.add('spread');
  if (haystack.contains('hummus')) tags.add('hummus');
  if (haystack.contains('guacamole')) tags.add('guacamole');
  if (haystack.contains('salsa')) tags.add('salsa');
  if (haystack.contains('tzatziki')) tags.add('tzatziki');
  if (haystack.contains('tapenade')) tags.add('tapenade');
  if (haystack.contains('muhammara')) tags.add('muhammara');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_snack_dips_review.csv';
