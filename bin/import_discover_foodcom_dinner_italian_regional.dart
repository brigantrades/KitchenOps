...
/// Italian favorites at-home ideas page — aligns with Discover "Italian"
/// (`dinner-italian`).
///
/// Source: https://www.food.com/ideas/italian-food-recipes-at-home-6828
///
/// Hub pages embed extra `/recipe/` links; JSON-LD is filtered to Italian / regional picks.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 80 — hub lists ~75)
///
/// Dry run writes tmp/foodcom_dinner_italian_regional_review.csv.
...
Future<void> main(List<String> args) async {
  ...
  final limit = _parseLimit(args) ?? 80;
  ...
  try {
    ...
    final urls = await _discoverRegionalItalianUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalItalianUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalItalianRecipe(jsonLd)) continue;
...
/// Ideas pages include unrelated `/recipe/` links; keep Italian / regional picks.
bool _isLikelyRegionalItalianRecipe(Map<String, dynamic> jsonLd) {
  ...
  if (cuisine.contains('italian')) return true;

  for (final s in _italianSignals) {
...
/// Tags tuned for [kBrowseCategories] `dinner-italian` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('pasta') || haystack.contains('spaghetti')) {
    tags.add('pasta');
  }
  if (haystack.contains('spaghetti')) tags.add('spaghetti');
  if (haystack.contains('penne')) tags.add('penne');
  if (haystack.contains('linguine')) tags.add('linguine');
  if (haystack.contains('risotto')) tags.add('risotto');
  if (haystack.contains('gnocchi')) tags.add('gnocchi');
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dinner_italian_regional_review.csv';
