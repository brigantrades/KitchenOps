...
/// Imports public dessert recipes from Food.com's "Sweet Small Bites" hub —
/// aligns with Discover "Sweet Bites" (`dessert-sweet-bites`).
///
/// Source: https://www.food.com/ideas/finger-food-desserts-6261
///
/// [meal_type] is `dessert` for Discover Desserts. Rows are tagged `sweet bites`
/// and `finger food` for browse matching.
///
/// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SEED_USER_ID
/// Args: [--execute] [--limit=N]  (default limit 32 — hub lists ~27)
///
/// Dry run writes tmp/foodcom_dessert_sweet_bites_review.csv.
...
const _ideasPageUrl =
    'https://www.food.com/ideas/finger-food-desserts-6261';
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final urls = await _discoverRegionalSweetBiteUrls(client, limit: limit);
...
Future<List<String>> _discoverRegionalSweetBiteUrls(
  http.Client client, {
  required int limit,
}) async {
  ...
  for (final candidate in orderedUrls) {
    ...
    try {
      ...
      if (!_isLikelyRegionalSweetBiteRecipe(jsonLd)) continue;
...
/// Curated finger-food dessert hub; accept any valid Recipe JSON-LD.
bool _isLikelyRegionalSweetBiteRecipe(Map<String, dynamic> _) {
  return true;
}

/// Tags tuned for [kBrowseCategories] `dessert-sweet-bites` plus dietary filters.
List<String> _buildCuisineTags({
  ...
  if (haystack.contains('mini')) tags.add('mini');
  if (haystack.contains('bite') || haystack.contains('bites')) {
    tags.add('bite-sized');
  }
...
Future<String> _writeReviewCsv(List<ReviewRow> rows) async {
  ...
  const path = 'tmp/foodcom_dessert_sweet_bites_review.csv';
