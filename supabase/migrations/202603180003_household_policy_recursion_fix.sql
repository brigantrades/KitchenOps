-- Fix RLS recursion on household_members by using a SECURITY DEFINER helper.
...
for select using (is_household_member(households.id));
...
for select using (is_household_member(household_members.household_id));
...
for insert with check (
  ...
  or (visibility = 'household' and user_id = auth.uid() and is_household_member(recipes.household_id))
